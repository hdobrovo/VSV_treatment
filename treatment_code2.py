...
for i, beta_val in enumerate(beta_values):
	for j, Vv0_val in enumerate(Vv0_values):
		...
		#	args=(beta_s, beta_v, k_s, k_v, delta_s, delta_v, p_s, p_v, c_s, c_v, gamma))
		...
		times_extended = np.linspace(0, 50, (50-0)*100)
		initial_conditions = [T0, Es0, Is0, Ev0, Iv0, Vs0, Vv0_val]
		...
		sol_extended = odeint(viral_infection, initial_conditions, times_extended, 
		...
		T_combined = sol_extended[:, 0]
		Es_combined = sol_extended[:, 1]
		Is_combined = sol_extended[:, 2]
		Ev_combined = sol_extended[:, 3]
		Iv_combined = sol_extended[:, 4]
		Vs_combined = sol_extended[:, 5]
		Vv_combined = sol_extended[:, 6]

		#Combining times
		times_combined = times_extended
...
plt.subplot(2, 3, 2)
plt.imshow((time_above_threshold_Vs_values), cmap='hot', extent=[-1,5,-1,5], aspect='auto', origin='lower', vmin = 0, vmax = 70)
plt.colorbar()
...
plt.subplot(2, 3, 5)
plt.imshow((time_above_threshold_Vv_values), cmap='hot', extent=[-1,5,-1,5], aspect='auto', origin='lower', vmin=0, vmax=70)
plt.colorbar()
...
plt.tight_layout()
plt.savefig("treatment0d-2.pdf", bbox_inches='tight', pad_inches=0.2)
plt.show()
